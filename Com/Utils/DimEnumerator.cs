        ...
        private List<DcColumn> GetNextValidSegments() // Here we find continuation segments that satisfy our criteria on columns
        {
            if (!isInverse) // Move up from lesser to greater
            {
                ...
                switch (dimType)
                {
                    ...
                    case DimensionType.GENERATING: return Output.Columns.Where(x => (x.GetData() != null && x.GetData().IsAppendData) && x.Input.Schema == x.Output.Schema).ToList();
                }
            }
            else
            {
                switch (dimType)
                {
                    ...
                    case DimensionType.GENERATING: return Input.InputColumns.Where(x => (x.GetData() != null && x.GetData().IsAppendData) && x.Input.Schema == x.Output.Schema).ToList();
                }
            }
